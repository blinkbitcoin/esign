...
- The current package names are `@blinkbitcoin/esign-core`,
  ...
  `packages/`; the backend is `packages/esign-service`. If these have changed, trust
...
|---|---------|------|-----------------|-------------|
| 1 | System Architecture | `flowchart TB` | `README.md` (modes) + `docs/architecture/integration.md`; routes from `packages/esign-service/src/app.ts` | `integration.md` (Parts) |
...
| 4 | Database ERD | `erDiagram` | `docs/architecture/data-models.md`; verify against `packages/esign-service/migrations/` | `data-models.md` (ERD section) |
| 5 | Component Hierarchy | `flowchart TB` | `docs/architecture/mobile.md` + demo `App.tsx` | `mobile.md` (Demo Host) |
| 6 | Webhook Flow | `sequenceDiagram` | `docs/architecture/backend.md` webhook section + `packages/esign-service/src/webhook.ts` | `backend.md` (Webhook Flow) |
| 7 | GraphQL Request Flow | `sequenceDiagram` | `docs/architecture/api-contracts.md` + `packages/esign-service/src/schema.ts` | `api-contracts.md` (createEnvelope) |
| 8 | Web Forms Mode Flow | `sequenceDiagram` | `docs/integration/webforms.md` + `packages/esign-service/src/providers/docusign/client.ts` | `webforms.md` (How it flows) |
...
2. `grep` the sources for stale identifiers: old package names, `Prisma`
   ...
   present in `packages/esign-service/src/app.ts`.
