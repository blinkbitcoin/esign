#!/usr/bin/env bash
# Writes packages/esign-service/.env for a live DocuSign run from the
...
#        WEBFORM_ID=… [PEM=packages/esign-service/.docusign.pem] [FORCE=1]
...
PEM="${PEM:-packages/esign-service/.docusign.pem}"
OUT="packages/esign-service/.env"
...
fi
{
  ...
  # No ESIGN_SESSION_SECRET: with ALLOW_INSECURE_DEV the bearer token is the user id,
  # which is what the live specs send (E2E_LIVE_AUTH_TOKEN). Set ESIGN_SESSION_SECRET=
