#!/usr/bin/env bash
# Builds the service image (packages/esign-service/Dockerfile) from the repo root with the
...
IMAGE="${1:?image name}"
ARCHIVE="${2:-}"
LABELS=(--label "org.opencontainers.image.title=esign-service")
...
docker build -f packages/esign-service/Dockerfile -t "$IMAGE" "${LABELS[@]}" .
if [ -n "$ARCHIVE" ]; then
