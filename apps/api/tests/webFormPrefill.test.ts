// The prefill contract at the edge: what POST /webform/instance lets through to
// the provider, and how the mock page renders each value shape.

import { formatPrefillValue, MAX_PREFILL_FIELDS, parseWebFormPrefill } from '../src/webFormPrefill';
...
describe('parseWebFormPrefill', () => {
  ...
  it.each([
    ...
    expect(parseWebFormPrefill(input)).toEqual({ ok: false, error: 'prefill must be an object' });
  });
  ...
  it.each([
    ...
    ['phone with non-string countryCode', { countryCode: 1, nationalNumber: '2' }],
  ...
  it('truncates a long offending field name in the error', () => {
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toBe(`invalid field name ${JSON.stringify('x'.repeat(100))}`);
    }
  });
  ...
  it('never lets a __proto__ key touch Object.prototype', () => {
    ...
    const input = JSON.parse('{"__proto__": {"polluted": "yes"}, "full_name": "Jane"}');
    const result = parseWebFormPrefill(input);
    // __proto__'s value is an object, which is not a supported value shape
    expect(result).toEqual({ ok: false, error: 'unsupported value for field "__proto__"' });
...
describe('formatPrefillValue', () => {
  it('renders each value shape the way a form would display it', () => {
    ...
    expect(formatPrefillValue({ countryCode: '55', nationalNumber: '1133301000' })).toBe(
      '+55 1133301000'
    );
