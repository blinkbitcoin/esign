...
import { escapeHtml, jsonForScript, sanitizeId } from '../src/html';
...
describe('sanitizeId', () => {
  it.each(['abc-123', 'A', 'a'.repeat(64), '0f1e2d3c-4b5a-6978-8a9b-0c1d2e3f4a5b'])(
    'passes the well-formed id %j through',
    (id) => {
      expect(sanitizeId(id)).toBe(id);
    }
  );

  it.each(['', 'a'.repeat(65), 'has space', 'semi;colon', '<img src=x onerror=alert(1)>', '../x'])(
    'replaces the malformed id %j with "unknown"',
    (id) => {
...
describe('jsonForScript', () => {
  ...
  it('keeps plain values as JSON.stringify would', () => {
    ...
    expect(jsonForScript({ a: 1, b: [true, null] })).toBe('{"a":1,"b":[true,null]}');
  });
});
