...
| 5 | DocuSign.js `sessionEnd` event: discriminator field (`type` / `sessionEndType` / `returnValue`) and values (`signingResult`, `formConfirmation`, `sessionTimeout`) | Web demo + browser devtools: log the raw event in the `sessionEnd` handler (temp `console.log` in `packages/esign-react/src/providers/docusign/webFormsSource.ts`), exercise finish + timeout | `interpretDocuSignEvent` in `packages/esign-core/src/providers/docusign/events.ts` + the mock page vocabulary in `packages/esign-server/src/pages.ts` |
