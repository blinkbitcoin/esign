...
| 5 | DocuSign.js `sessionEnd` event: discriminator field (`type` / `sessionEndType` / `returnValue`) and values (`signingResult`, `formConfirmation`, `sessionTimeout`) | Web demo + browser devtools: log the raw event in the `sessionEnd` handler (temp `console.log` in `packages/esign-react/src/docusignWebForms.ts`), exercise finish + timeout | `interpretDocuSignEvent` in `packages/esignature-core/src/signing/events.ts` + the mock page vocabulary in `apps/api/src/signingPages.ts` |
