...
e2e-android-local` / `make e2e-ios-local` (the whole Maestro stack on a
laptop, torn down on exit; `make e2e-android` / `make e2e-ios` are the
suites alone, `make e2e-backend-up` / `android-build` / `ios-build` /
`e2e-metro-up` the steps), `make db-up/migrate/backend`,
