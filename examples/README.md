...
take on `@blinkbitcoin/esign-node`.
...
| [`full-service-demo/`](full-service-demo/README.md) | 🖥️ `@blinkbitcoin/esign-node` as a whole service: Express router + Apollo + Postgres store + webhooks (the backend the two client demos and every E2E suite run against; ships as the `esign-api` image) |
